...
#ifndef MJPC_TASKS_BIGREDDOG_BIGREDDOG_H_
#define MJPC_TASKS_BIGREDDOG_BIGREDDOG_H_
...
class BigRedDogFlat : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const BigRedDogFlat* task)
...
class BigRedDogHill : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const BigRedDogHill* task, int current_mode = 0)
    ...
    void Residual(const mjModel* model, const mjData* data,
                  double* residual) const override;
   private:
    friend class BigRedDogHill;
    int current_mode_;
  };
  BigRedDogHill() : residual_(this) {}
  void TransitionLocked(mjModel* model, mjData* data) override;
...
#endif  // MJPC_TASKS_REDDOG_REDDOG_H_
