...
#include "mjpc/tasks/bigreddog/bigreddog.h"
...
namespace mjpc {
std::string BigRedDogHill::XmlPath() const {
  return GetModelPath("bigreddog/task_hill.xml");
}
std::string BigRedDogFlat::XmlPath() const {
  return GetModelPath("bigreddog/task_flat.xml");
}
std::string BigRedDogHill::Name() const { return "BigReddog Hill"; }
std::string BigRedDogFlat::Name() const { return "BigReddog Flat"; }

void BigRedDogFlat::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
...
//  ============  transition  ============
void BigRedDogFlat::TransitionLocked(mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
...
void BigRedDogFlat::ModifyScene(const mjModel* model, const mjData* data,
...
void BigRedDogFlat::ResetLocked(const mjModel* model) {
  // ----------  task identifiers  ----------
  ...
  for (const char* shouldername : {"FL_hip_link", "RL_hip_link", "FR_hip_link", "RR_hip_link"}) {
...
void BigRedDogFlat::ResidualFn::AverageFootPos(
...
double BigRedDogFlat::ResidualFn::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void BigRedDogFlat::ResidualFn::Walk(double pos[2], double time) const {
...
// get gait
BigRedDogFlat::ResidualFn::A1Gait BigRedDogFlat::ResidualFn::GetGait() const {
  if (current_mode_ == kModeBiped)
    return kGaitTrot;
...
double BigRedDogFlat::ResidualFn::StepHeight(double time, double footphase,
...
void BigRedDogFlat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                         A1Gait gait) const {
...
// height during flip
double BigRedDogFlat::ResidualFn::FlipHeight(double time) const {
...
void BigRedDogFlat::ResidualFn::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2*mjPI;
...
// -----------------------------------------------------------------------
void BigRedDogHill::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
                                         double* residual) const {
  // ---------- Residual (0) ----------
...
// -----------------------------------------------
void BigRedDogHill::TransitionLocked(mjModel* model, mjData* data) {
