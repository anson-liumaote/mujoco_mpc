...
namespace mjpc {
std::string RedDogHill::XmlPath() const {
  return GetModelPath("reddog/task_hill.xml");
}
std::string RedDogFlat::XmlPath() const {
  return GetModelPath("reddog/task_flat.xml");
}
std::string RedDogHill::Name() const { return "Reddog Hill"; }
std::string RedDogFlat::Name() const { return "Reddog Flat"; }

void RedDogFlat::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
...
//  ============  transition  ============
void RedDogFlat::TransitionLocked(mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
...
void RedDogFlat::ModifyScene(const mjModel* model, const mjData* data,
...
void RedDogFlat::ResetLocked(const mjModel* model) {
  // ----------  task identifiers  ----------
...
void RedDogFlat::ResidualFn::AverageFootPos(
...
double RedDogFlat::ResidualFn::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void RedDogFlat::ResidualFn::Walk(double pos[2], double time) const {
...
// get gait
RedDogFlat::ResidualFn::A1Gait RedDogFlat::ResidualFn::GetGait() const {
  if (current_mode_ == kModeBiped)
    return kGaitTrot;
...
double RedDogFlat::ResidualFn::StepHeight(double time, double footphase,
...
void RedDogFlat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                         A1Gait gait) const {
...
// height during flip
double RedDogFlat::ResidualFn::FlipHeight(double time) const {
...
void RedDogFlat::ResidualFn::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2*mjPI;
...
// -----------------------------------------------------------------------
void RedDogHill::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
                                         double* residual) const {
  // ---------- Residual (0) ----------
...
// -----------------------------------------------
void RedDogHill::TransitionLocked(mjModel* model, mjData* data) {
