...
#ifndef MJPC_TASKS_REDDOG_REDDOG_H_
#define MJPC_TASKS_REDDOG_REDDOG_H_
...
class RedDogFlat : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const RedDogFlat* task)
   ...
   private:
    ...
    constexpr static double kHeightQuadruped = 0.2;  // meter
...
class RedDogHill : public Task {
 public:
  std::string Name() const override;
  std::string XmlPath() const override;
  class ResidualFn : public mjpc::BaseResidualFn {
   public:
    explicit ResidualFn(const RedDogHill* task, int current_mode = 0)
    ...
    void Residual(const mjModel* model, const mjData* data,
                  double* residual) const override;
   private:
    friend class RedDogHill;
    int current_mode_;
  };
  RedDogHill() : residual_(this) {}
  void TransitionLocked(mjModel* model, mjData* data) override;
